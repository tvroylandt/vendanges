...
dads_demo_comparaison <- dads_demo %>%
  ...
  mutate(part_n = n / sum(n) * 100,
         part_n = if_else(SEXE == "Femme", -part_n, part_n)) %>%
  filter(AGE_TR != "00" & !is.na(AGE_TR)) %>%
  ungroup()

# Fusion
dads_demo_comparaison %>% 
  ggplot(aes(x = AGE_TR, fill = SEXE, y = part_n)) +
  geom_bar(
    stat = "identity",
    position = "dodge",
    width = 2
  ...
  scale_x_discrete("") +
  scale_y_continuous(name = "Part des emplois (en %)",
                     breaks = seq(-12, 14, 2),
                     labels = paste0(as.character(c(seq(12, 0, -2), seq(2, 14, 2))))) +
  scale_fill_manual("Sexe",
                    values = c("#00baff", "#ff4500")) +
  coord_flip() +
