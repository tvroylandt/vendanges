...
# OPTION 1 - Date médiane par département
# Avec sélection des départements et flèche
# Jointure
stats_dep_shp <- shp_dep %>%
  filter(n > 30 & date_deb_med > 230) %>%
  left_join(stats_dep, by = c("DEP" = "DEPT"))

# Centroids pour label
label_dep <- stats_dep_shp %>%
  st_centroid %>%
  st_coordinates() %>%
  as_tibble() %>%
  bind_cols(date_deb_med = stats_dep_shp$date_deb_med) %>%
  filter(!is.na(date_deb_med))

# Graph
ggplot() +
  geom_sf(data = stats_dep_shp, aes(fill = date_deb_med)) +
  geom_text(data = label_dep,
            aes(x = X, y = Y, label = date_deb_med),
            size = 3) +
  scale_fill_gradient(
    name = "Date médiane \nde début d'emploi",
    low = "#ffb59a",
...
```{r saisonnalite_geo_2, eval = FALSE}
# OPTION 2 - Progression de l'emploi
...
decile_dep_shp <- shp_dep %>%
  ...
  mutate(decile = fct_explicit_na(decile, ""),
         decile = fct_recode(decile,
                             ...
                             "80 %" = "d8",
                             "90 %" = "d9")
         val_decile = if_else(is.na(val_decile), 0, val_decile))
...
anim_carte <- ggplot() +
  ...
  transition_manual(val_decile, 
                    cumulative = TRUE) +
  labs(title = "Jour : {current_frame}") +
  theme_void()
